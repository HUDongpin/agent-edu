...
test("discovers a stateless 2026-07-28 server", () => {
  ...
  assert.equal(response.result.ttlMs, 60_000);
...
test("includes mandatory caching hints on every implemented cacheable Complete result", () => {
  const cacheableRequests = [
    request(20, "server/discover"),
    request(21, "tools/list"),
    request(22, "prompts/list"),
    request(23, "resources/list"),
    request(24, "resources/read", { uri: "courseops://courses/mcp-10" }),
  ];

  for (const cacheableRequest of cacheableRequests) {
    const response = handleForTest(cacheableRequest);
    assert.equal(response.result.resultType, "complete");
    assert.equal(Number.isFinite(response.result.ttlMs), true);
    assert.match(response.result.cacheScope, /^(public|private)$/);
  }
});
