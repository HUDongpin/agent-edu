...
| Nine complete machine catalogs with no unexplained fallback | `messages/`, `messages/widgets/`, `messages/handbook/`, narrow allowlist; 24-file reviewer inventory in `docs/release/evidence/native-review-catalog-precheck-2cdf1d6.json` | `npm run handbook:check`, `npm run widgets:check`, `npm run release:check` localization phase; privacy-safe catalog-manifest contract | Automatic structural pass and reviewer inputs frozen by SHA-256/key count; eight native-language judgments and final-candidate digest confirmation remain external pending |
