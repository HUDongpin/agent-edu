...
| Fake key, Prompt, reply and raw Provider data stay out of persistent/exported surfaces | Session-only key, restricted draft codec, redaction, fail-closed test routing and artifact scanner for filesystem/ZIP/embedded-report contents and names | Secrets/privacy unit tests; Provider contract browser tests; `npm run artifacts:check` | Implemented; CI failure-artifact behavior still requires frozen GitHub run evidence |
