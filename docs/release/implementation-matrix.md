...
| Nine complete machine catalogs with no unexplained fallback | `messages/`, `messages/widgets/`, `messages/handbook/`, narrow allowlist; historical 24-file precheck in `docs/release/evidence/native-review-catalog-precheck-2cdf1d6.json`; final-candidate generator/checker in `scripts/native-review-inventory.mjs` | `npm run handbook:check`, `npm run widgets:check`, `npm run native-review:inventory:check`, `npm run release:check` localization phase; privacy-safe exact-file candidate/workflow contract | Automatic structural pass; deterministic final-candidate inventory gate implemented but deliberately unmaterialized until the final SHA is frozen. Eight native-language judgments remain external pending |
