...
**Status:** approved specification, awaiting implementation. Nothing named `cost` exists in the
repository today beyond the catalogue reservation: `lib/courses.ts` carries
...
already holds a `cost` motif commented "a falling stack — cost coming down". There is no route,
no component, no plan module, no strings namespace and no test. This document is the
...
**Decision this brief makes before anything else: Cost Engineering is a fourth catalogue course,
