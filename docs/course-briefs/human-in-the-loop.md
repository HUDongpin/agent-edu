...
**Status:** approved specification, awaiting implementation. No code exists. `lib/courses.ts`
carries the catalogue row (`id: "hitl"`, `href: "#"`, `status: "soon"`) and
`components/courses/Cover.tsx` carries a cover motif; nothing else in the repository
implements this course. This document replaces the one-line blurb as the specification, and
ends the loop in which `README.md` calls human-in-the-loop design a gap "already named as
gaps in the catalogue" while `lib/courses.ts` describes its own `soon` entries as "the gaps
the README already names". An approved brief is not permission to publish.
...
`handbook`, `lab` or `build`, and `tests/progress.test.ts:686` asserts exactly that object,
