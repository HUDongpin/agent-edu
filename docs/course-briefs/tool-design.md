...
still carries `href: "#"` and `status: "soon"`; `messages/*.json` carries `c.tools.title` and
`c.tools.blurb` in all nine locales and nothing else; there is no `course/tools/` directory. This
document is the specification that `lib/courses.ts` says the README holds and that `README.md`
says the catalogue holds. Neither did. This does.
...
Fixed and not open for revision: 60 minutes, format `code`, topic `agents`, level `advanced`, hue
