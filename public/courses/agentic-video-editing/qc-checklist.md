# Course 22 candidate review checklist / 课程 22 候选成片审校清单

This is an original, offline teaching fixture for a fictional edit. It is a decision record, not an automated quality score and not publication authority. Complete it against one frozen, hash-bound candidate render and one frozen, hash-bound review packet. A well-supported **do not publish / 不发布** decision is a valid outcome.

这是用于虚构剪辑的原创离线教学 fixture。它是决策记录，不是自动质量分，也不构成发布授权。必须针对一个已冻结并由 hash 绑定的候选成片，以及一个已冻结并由 hash 绑定的审校包填写。证据充分的 **不发布 / do not publish** 是有效结果。
...
Hash the exact frozen UTF-8 bytes of `edit-plan.json`; approvals live outside that byte domain. Freeze and hash the review packet before creating this decision record, and exclude this decision record from the review-packet hash. If any bound hash changes, this checklist is stale and must be repeated. The agent may prepare evidence but may not fill the named-human decision or approve its own work.

对已冻结 `edit-plan.json` 的准确 UTF-8 字节计算 hash；批准记录位于该字节域之外。先冻结并 hash 审校包，再创建本决定记录；本决定记录不得计入审校包 hash。任何绑定摘要变化都会使本清单失效，必须重新审校。智能体可以准备证据，但不得代填具名人类决定，也不得自我批准。
...
- [ ] Input hashes, probe receipts, frame rate, timebase, rotation, color metadata, channels, and duration match the approved manifest. / 输入摘要、探测回执、帧率、时间基准、旋转、色彩元数据、声道与时长均匹配已批准清单。
- [ ] Every executed operation has a stable operation ID, source media ID, frame range, reason, evidence reference, confidence, ambiguity state, and human-review state. / 每个已执行操作均有稳定操作 ID、源媒体 ID、帧范围、理由、证据引用、置信度、歧义状态与人审状态。
...
## G. Named human release decision / 具名人类发布决定
...
- [ ] **blocked / 阻断** — do not publish; unresolved items / 不得发布；未解决事项：`________________________________`
