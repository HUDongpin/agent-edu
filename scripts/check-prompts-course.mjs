...
function loadTypeScriptCourseData() {
  ...
  const expression = [
    ...
    'import { PROMPT_CAPSTONE_KEY, PROMPT_CAPSTONE_REQUIRED_KEY, PROMPT_CAPSTONE_SCORES_KEY, PROMPT_QUIZ_BANK_VERSION, PROMPT_QUIZ_BEST_KEY, PROMPT_QUIZ_MAX_SCORE, PROMPT_QUIZ_VERSION_KEY } from "./lib/prompts/progress-keys.ts";',
    'import { PROMPT_CAPSTONE_REQUIRED_COUNT, PROMPT_CAPSTONE_RUBRIC_COUNT, promptCapstoneScoresPass } from "./lib/prompts/capstone.ts";',
    ...
    'completeProgress[PROMPT_QUIZ_VERSION_KEY] = PROMPT_QUIZ_BANK_VERSION;',
    'completeProgress[PROMPT_QUIZ_BEST_KEY] = PROMPT_QUIZ_MAX_SCORE;',
    'completeProgress["prompts.quiz.passed"] = true;',
    'completeProgress[PROMPT_CAPSTONE_REQUIRED_KEY] = Object.fromEntries(Array.from({ length: PROMPT_CAPSTONE_REQUIRED_COUNT }, (_, index) => [index, true]));',
    'completeProgress[PROMPT_CAPSTONE_SCORES_KEY] = Object.fromEntries(Array.from({ length: PROMPT_CAPSTONE_RUBRIC_COUNT }, (_, index) => [index, 2]));',
    'completeProgress[PROMPT_CAPSTONE_KEY] = true;',
    'const staleCapstoneProgress = { ...completeProgress, [PROMPT_CAPSTONE_KEY]: false, "prompts.capstone.v1": true };',
