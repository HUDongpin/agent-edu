...
function assertDynamicParamsFalse(source, file) {
  assert(
    /export\s+const\s+dynamicParams\s*=\s*false\s*;/.test(source),
    `${file} must export dynamicParams = false`,
  );
}
...
function assertRouteWiring(course) {
  ...
  for (const root of roots) {
    ...
    assertDynamicParamsFalse(source, file);
    if (authored) {
      assert(
