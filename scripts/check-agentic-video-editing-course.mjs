...
/**
 * Deterministic, offline, fail-closed gate for Course 22.
 *
...
const EXPECTED_SOURCE_LEDGER_SHA256 = "ade90e834a8e5ee30c92dc5c650c2d5314005b90e46e077ea53b0c36d8b96dff";
const EXPECTED_GITHUB_SNAPSHOTS = new Map([
  ...
  ["remotion", ["remotion-dev/remotion", "release-pinned", "v4.0.518 (lightweight tag)", "ac804a72e09c97e79ae1bbae49f8a12e7beacd0c", "Remotion License (special commercial terms)"]],
  ...
  ["montaj", ["theSamPadilla/montaj", "release-pinned", "v4.2.0 (lightweight tag)", "b63e46768029ffb12f6f06ff9432ac1a73403c9b", "MIT"]],
  ["timeline-studio", ["MartinDelophy/ai-video-editor", "release-pinned", "release tag v1.0.6 (lightweight); package.json name web-player version 1.0.0", "ed010e815a8b686e7cc0f349786a0af012764785", "MIT"]],
  ["qcut", ["Quriosity-agent/qcut", "release-pinned", "v2026.08.26.3", "a6ae6aed5d6546687d3b1c753a0195608b23d855", "MIT for qcut source tree; providers and assets remain separate"]],
  ["velorn", ["VelornLabs/velorn", "release-pinned", "v0.3.30", "05240b9aa971a24e19a51247ddae7fda919b1795", "GPL-3.0-only"]],
...
async function importFresh(path) {
  return import(`${pathToFileURL(path).href}?course22-check=${Date.now()}`);
}
...
export async function checkAgenticVideoEditingCourse({
  ...
  if (courseModule) {
    ...
    const guidedProject = validateAgenticVideoEditingGuidedProject(
      abs(`${ASSET_DIRECTORY}/lab/fixtures/guided-v2`),
      { validatedAt: "2026-08-28T00:00:00+08:00" },
    );
    if (!guidedProject.ok) {
      for (const issue of guidedProject.issues) add("artifacts", `guided validator: ${issue}`);
    }
    if (guidedProject.fixtureLedgerSha256 !== GUIDED_LEDGER_SHA256
      || AGENTIC_VIDEO_EDITING_FIXTURE_LEDGER_SHA256 !== GUIDED_LEDGER_SHA256) {
      add("artifacts", "guided ledger and preflight hash must equal the locked Course 22 ledger.");
    }

    if (!manifest || !sources || !enCopy || !zhHansCopy) {
      add("course-definition", "Course 22 must export its manifest, sources, and both authored candidate copy bundles.");
    } else {
      try {
      ...
      if (manifest.displayNumber !== 22) add("course-definition", `Display number drifted to ${manifest.displayNumber}.`);
      ...
      if (!same([...AGENTIC_VIDEO_EDITING_MODULE_SLUGS], EXPECTED_MODULE_SLUGS)
        ...
        add("course-definition", "Module slugs must match the independent Course 22 order exactly.");
      }
      ...
      for (const requiredKind of ["github-repository", "x-post", "open-standard", "legal-policy", "community-issue"]) {
        if (!sourceKinds.has(requiredKind)) add("sources", `Required source class ${requiredKind} is absent.`);
      }
      ...
      if (sourceLedgerHash !== EXPECTED_SOURCE_LEDGER_SHA256) {
        add("sources", `Source ledger content drifted from the locked local integrity snapshot (${sourceLedgerHash}).`);
      }
      ...
      for (const source of sources) {
        ...
        } else if (["official-documentation", "open-standard", "legal-policy", "community-issue"].includes(source.kind)) {
          if (source.accessedAt !== "2026-08-28T00:00:00+08:00") {
            add("sources", `${source.id}: official/standard/policy/community evidence needs the offset-aware access timestamp.`);
          }
          if (source.stability === "version-pinned-standard" && !source.immutableRef) {
            add("sources", `${source.id}: version-pinned standard needs a genuine immutableRef.`);
          }
          if (source.stability !== "version-pinned-standard" && source.immutableRef) {
            add("sources", `${source.id}: rolling or observation-only evidence must not claim an immutableRef.`);
          }
          if ((source.kind === "community-issue") && source.evidenceUse !== "version-watch-only") {
            add("sources", `${source.id}: community issue must be version-watch-only.`);
          }
        } else {
      ...
      for (const [locale, copy] of reviewedCopies) {
        ...
        if (!same([...copySlugs].sort(), [...EXPECTED_MODULE_SLUGS].sort())) {
          add("language", `${locale}: authored candidate copy must contain the ten canonical modules.`);
        }
        ...
        if (copy.capstone.artifacts.length !== 12
          || duplicateValues(copy.capstone.artifacts.map((artifact) => artifact.artifactId)).length
          || copy.capstone.artifacts.some((artifact) => artifact.label.trim().length < minimumArtifactLength)) {
      ...
      for (const locale of EXPECTED_LOCALES) {
        try {
          ...
          if (locale !== "en" && locale !== "zh-Hans"
            && !/(?:authored English candidate|candidate bundle)/iu.test(materialized.copy.meta.translationNote)) {
            add("language", `${locale}: fallback route must disclose the authored English candidate and pending review HOLD.`);
          }
        } catch (error) {
  ...
  if (!existsSync(assetDirectory)) {
    ...
    if (!directoryStat.isDirectory() || directoryStat.isSymbolicLink()) {
      ...
      const expectedEntries = [...ALL_PUBLIC_FILES, "lab"].sort((left, right) => left.localeCompare(right, "en"));
      ...
      for (const entry of entries) {
        if (entry !== "lab") regularFile(join(assetDirectory, entry), `${ASSET_DIRECTORY}/${entry}`);
      }
    }
  }
  ...
  if (provenance) {
    ...
    if (scope.fictionalScenario !== true
      || scope.shipsSourceMedia !== true
      || scope.sourceMediaBoundary !== "course-owned-synthetic-only"
      ...
      add("rights", "Fixture provenance must remain fictional, limited to course-owned synthetic media, non-authorizing, human-released, and fail-closed on unknown rights.");
    }
    const guidedProvenance = provenance.guidedProject ?? {};
    if (guidedProvenance.ledgerPath !== "lab/fixtures/guided-v2/guided-project.ledger.json"
      || guidedProvenance.ledgerSha256 !== GUIDED_LEDGER_SHA256
      || guidedProvenance.archivePath !== "lab/course22-guided-v2.zip"
      || guidedProvenance.archiveSha256 !== sha256(join(assetDirectory, "lab/course22-guided-v2.zip"))
      || guidedProvenance.playableSourceMedia !== true
      || guidedProvenance.playableCpuRender !== true
      || guidedProvenance.networkRequired !== false
      || guidedProvenance.credentialsRequired !== false
      || guidedProvenance.publicationAuthority !== false) {
      add("rights", "Guided provenance must bind the playable synthetic-media ledger while denying network, credential, and publication authority.");
    }
  ...
  if (editPlanSchema) {
    ...
    const candidateSetRef = editPlanSchema.properties?.candidateSetRef ?? {};
    ...
    for (const field of ["schemaVersion", "planId", "planMode", "executionMode", "status", "inputs", "timeline", "executionPolicy", "candidateSetRef"]) {
    ...
    if (editPlanSchema.$schema !== "https://json-schema.org/draft/2020-12/schema"
      ...
      || Object.hasOwn(editPlanSchema.properties ?? {}, "analysisLanes")
      ...
      || candidateSetRef.additionalProperties !== false
      || candidateSetRef.properties?.artifactId?.const !== "candidate-segments"
      || candidateSetRef.properties?.artifactSchemaId?.const
        !== "aicourse.agentic-video-editing.module.semantic-analysis-director.artifact.v2"
      || candidateSetRef.properties?.artifactSha256?.$ref !== "#/$defs/sha256"
      || teachingBranch.fixtureId?.const !== "course22-cut-plan-lab-v2"
      || teachingBranch.candidateSetRef?.properties?.artifactSha256?.const
        !== "ee16eac9a48eaf0c80a7e4a2774e216481a1cee29481353b83b9a64a756dc5ef"
    ...
    try {
      ...
      if (!schemaFixturePlan) {
        ...
        productionPlan.candidateSetRef = {
          projectId: "production-candidate-project-v2",
          artifactId: "candidate-segments",
          artifactSchemaId: "aicourse.agentic-video-editing.module.semantic-analysis-director.artifact.v2",
          artifactSha256: "d".repeat(64),
          locator: "artifacts/candidate-segments.json",
          selectedLaneId: "manual-review",
        };
  ...
  requireTokens(`${ASSET_DIRECTORY}/NOTICE.md`, [
    "original fixtures",
    "course-generated color video",
    "course-owned synthetic media",
    "third-party code",
    "link-and-paraphrase-only",
    ...
    "SHA-256",
    "do not authenticate learner-media rights",
  ], "rights");
  ...
  if (release) {
    const localizationReviews = readJson("lib/agentic-video-editing/localization-reviews.json");
    const bundles = localizationReviews?.bundles ?? [];
    if (localizationReviews?.courseVersion !== COURSE_VERSION || bundles.length !== 2) {
      add("release-review", "Course 22 requires exact EN and zh-Hans localization review records.");
    }
    for (const bundle of bundles) {
      const copyPath = abs(bundle.path ?? "");
      const copyHash = existsSync(copyPath) && lstatSync(copyPath).isFile()
        ? createHash("sha256").update(readFileSync(copyPath)).digest("hex")
        : null;
      if (copyHash !== bundle.sha256) {
        add("release-review", `${bundle.locale}: localization review hash is stale.`);
      }
      if (bundle.status !== "approved" || !bundle.reviewerId || !bundle.reviewedAt) {
        add("release-review", `${bundle.locale}: named-human bundle review is ${bundle.status ?? "missing"}; release remains HOLD.`);
      }
    }
    const packageJson = readJson("package.json");
    if (packageJson) {
      const scripts = packageJson.scripts ?? {};
      const expectedContentCommand = "node --import tsx scripts/check-agentic-video-editing-course.mjs --local";
      const expectedReleaseCommand = "node --import tsx scripts/check-agentic-video-editing-course.mjs --release";
      ...
      if (packageJson.devDependencies?.ajv !== "8.20.0") {
        add("release", "Course 22 schema gate requires the exact Ajv 8.20.0 Draft 2020-12 validator dependency.");
      }
      ...
      if (String(scripts.build ?? "").includes(`${COURSE_ID}:check:release`)) {
        add("release", "Ordinary build must remain a candidate build and must not claim Course 22 release approval.");
      }
      if (!String(scripts["build:release"] ?? "").startsWith(`npm run ${COURSE_ID}:check:release && `)) {
        add("release", "build:release must fail closed on the Course 22 named-human localization approval gate.");
      }
    }
    ...
    if (browserConfig.includes("PLAYWRIGHT_BASE_URL")) {
      add("release", "Course 22 release browser config must not permit an external base URL to bypass the candidate out/ server.");
    }
    ...
    try {
      ...
      if (!top || top.displayNumber !== 22 || top.href !== `/${COURSE_ID}/`
        || top.status !== "available" || top.minutes !== 750
        || !same([...top.moduleIds], EXPECTED_MODULE_SLUGS)
        || top.progressStrategy !== "twelve-equal-milestones") {
        add("release", "Course 22 top-level registration must be available, numbered 22, 750 minutes, linkable, and closed over ten module routes.");
      }
      if (!catalog || catalog.displayNumber !== 22 || catalog.href !== `/${COURSE_ID}/`
        || catalog.status !== "available" || catalog.minutes !== 750
        || catalog.contentLanguageKey !== `c.${COURSE_ID}.contentLanguage`) {
        add("release", "Course 22 catalogue registration must expose its reviewed-content language disclosure.");
      }
      const expectedPages = [
        `${COURSE_ID}/`,
  ...
  if (!issues.length) {
    note("Every required source class is present, cited, and bounded by machine-readable evidence-use rules; no quota substitutes for claim closure.");
    note("Four phases, modules 1–10, 750 minutes, two authored candidate bundles, assessment, capstone, and progress contracts close; bundle review remains a separate release HOLD.");
    note("Five top-level learning files plus the playable synthetic-media guided ledger are regular, rights-bounded, and fail-closed; no learner media or reviewer identity is authenticated.");
  ...
  return {
    ...
    localContract: issues.filter((issue) => issue.gate !== "release-review" && issue.gate !== "release").length
      ? "FAIL"
      : "PASS",
    release: release ? (issues.length ? "HOLD" : "ELIGIBLE") : "NOT EVALUATED",
    releaseEligible: release && issues.length === 0,
    skippedGates: release ? [] : ["localization-approval"],
...
export function formatAgenticVideoEditingCheck(result) {
  const lines = [
    `local contract: ${result.localContract}`,
    `release: ${result.release}`,
    `releaseEligible=${String(result.releaseEligible)}`,
    `skippedGates=[${result.skippedGates.join(",")}]`,
...
if (invoked) {
  ...
  if (moduleSlug || artifactId || artifactPath) {
    if (!moduleSlug || !artifactId || !artifactPath) {
      console.error("--module, --artifact-id, and --artifact must be supplied together");
      process.exitCode = 1;
    } else {
      const learnerRoot = valueAfter("--learner-final");
      const guidedWorkingRoot = valueAfter("--guided-project");
      const validatedAt = valueAfter("--validated-at") ?? new Date().toISOString();
      if (learnerRoot && guidedWorkingRoot) {
        console.error("--learner-final and --guided-project are mutually exclusive");
        process.exitCode = 1;
        process.exit();
      }
      const project = learnerRoot
        ...
        : validateAgenticVideoEditingGuidedProject(
          guidedWorkingRoot
            ? resolve(guidedWorkingRoot)
            : resolve(DEFAULT_ROOT, ASSET_DIRECTORY, "lab/fixtures/guided-v2"),
          {
            receiptPathRoot: process.cwd(),
            validatedAt,
            verifyFixtureLedger: !guidedWorkingRoot,
            throughModuleSlug: moduleSlug,
          },
        );
      const receipts = project.receiptArrays[moduleSlug] ?? [];
      const selected = receipts.find((receipt) => receipt.artifactId === artifactId);
      if (!project.ok || !selected || selected.artifactPath !== artifactPath) {
        for (const issue of project.issues) console.error(`- ${issue}`);
        console.error(`No validated receipt for ${moduleSlug}/${artifactId}/${artifactPath}`);
        process.exitCode = 1;
      } else {
