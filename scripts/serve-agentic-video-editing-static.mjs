...
/**
 * Minimal static-export server for Course 20 browser acceptance.
 *
...
import { checkCourse20StaticExport } from "./check-agentic-video-editing-static.mjs";
...
const staticAudit = checkCourse20StaticExport();
if (staticAudit.errors.length) {
  throw new Error(
    `Refusing to serve an unverified Course 20 export (${staticAudit.errors.length}):\n- ${staticAudit.errors.join("\n- ")}`,
  );
}
...
server.listen(PORT, HOST, () => {
  process.stdout.write(`Course 20 static export server listening on http://${HOST}:${PORT}\n`);
});
