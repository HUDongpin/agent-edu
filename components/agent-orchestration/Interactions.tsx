...
function useProgress() {
  ...
  useEffect(() => {
    const refresh = () => setRecord(readAgentOrchestrationProgress());
    const refreshFromStorage = (event: StorageEvent) => {
      if (isAgentOrchestrationProgressStorageEvent(event)) {
        refresh();
      }
    };
    refresh();
    window.addEventListener(AGENT_ORCHESTRATION_PROGRESS_EVENT, refresh);
    window.addEventListener(AGENT_ORCHESTRATION_PROGRESS_RESET_EVENT, refresh);
    window.addEventListener("storage", refreshFromStorage);
    return () => {
      window.removeEventListener(AGENT_ORCHESTRATION_PROGRESS_EVENT, refresh);
      window.removeEventListener(AGENT_ORCHESTRATION_PROGRESS_RESET_EVENT, refresh);
...
function isChinese(labels: Labels): boolean {
  return labels.module === "模块";
}

function persistenceText(labels: Labels, persisted: boolean | null): string {
  if (persisted === null) return "";
  if (persisted) return label(labels, "savedLocally", isChinese(labels) ? "已保存在此浏览器" : "Saved in this browser");
  return label(labels, "memoryOnly", isChinese(labels) ? "仅保存在当前标签页" : "Saved for this tab only");
}
...
export function CourseProgress({
  ...
  useEffect(() => {
    const refresh = () => setStorageReady(isAgentOrchestrationStorageAvailable());
    const refreshAfterChange = () => {
      refresh();
      setResetResult(null);
    };
    const timer = window.setTimeout(refresh, 0);
    window.addEventListener(AGENT_ORCHESTRATION_PROGRESS_EVENT, refreshAfterChange);
    window.addEventListener(AGENT_ORCHESTRATION_PROGRESS_RESET_EVENT, refresh);
    window.addEventListener("storage", refreshAfterChange);
    return () => {
      window.clearTimeout(timer);
      window.removeEventListener(AGENT_ORCHESTRATION_PROGRESS_EVENT, refreshAfterChange);
      window.removeEventListener(AGENT_ORCHESTRATION_PROGRESS_RESET_EVENT, refresh);
      window.removeEventListener("storage", refreshAfterChange);
    };
  }, []);

  return (
    <section className={compact ? styles.progressCompact : styles.progressPanel}>
          ...
          <small role="status" aria-live="polite">
            {resetResult !== null
              ? (resetResult
                ...
                : label(labels, "resetMemory", isChinese(labels) ? "课程 15 进度已在当前标签页重置" : "Course 15 progress reset for this tab"))
              : storageReady === null
                ? label(labels, "checkingStorage", isChinese(labels) ? "正在检查保存能力…" : "Checking persistence…")
                : storageReady
              ? label(labels, "savedLocally", "Saved in this browser")
                  : label(labels, "memoryOnly", isChinese(labels) ? "隐私模式：进度仅保留在当前标签页" : "Private browsing: progress lasts for this tab")}
          </small>
          <button
            type="button"
            onClick={() => {
              ...
              const persisted = resetAgentOrchestrationProgress();
              setResetResult(persisted);
              setResetArmed(false);
            }}
          >
            {resetArmed
...
export function ArtifactWorkbench({
  ...
  const [draftResult, setDraftResult] = useState<boolean | null>(null);
  ...
  }, [pendingDraft, practice.template, storedArtifact]);
  ...
  return (
    <div className={styles.workbench}>
      ...
      <small className={styles.persistenceStatus} role="status" aria-live="polite">
        {saveResult !== null
          ? `${label(labels, "artifactEvidenceSaved", isChinese(labels) ? "证据收据已保存。" : "Evidence receipt saved.")} ${persistenceText(labels, saveResult)}`
          : draftResult !== null
            ? `${label(labels, "draftAutoSaved", isChinese(labels) ? "工作草稿已自动保存；尚不计入完成证据。" : "Working draft auto-saved; it does not yet count as completion evidence.")} ${persistenceText(labels, draftResult)}`
            : typeof pendingDraft === "string"
              ? label(labels, "draftRecovered", isChinese(labels) ? "已恢复工作草稿；请通过证据门并显式保存。" : "Working draft restored; pass the evidence gate and save explicitly.")
              : ""}
      </small>
    </div>
  );
...
export function ModuleCheckpoint({
  ...
  const storageKey = agentOrchestrationCheckpointKey(slug);
  const passedKey = agentOrchestrationCheckpointPassedKey(slug);
  const progress = useProgress();
  const [choice, setChoice] = useState<number | null>(null);
  const [checked, setChecked] = useState(false);
  const [saveResult, setSaveResult] = useState<boolean | null>(null);
  const correct = choice === checkpoint.correctIndex;
  const storedChoice = progress[storageKey];

  useEffect(() => {
    const timer = window.setTimeout(() => {
      if (typeof storedChoice === "number") {
    ...
    <section className={styles.checkpoint} aria-labelledby={`${slug}-checkpoint-title`}>
      ...
      <button
        ...
        onClick={() => {
          ...
          setChecked(true);
          const persisted = updateAgentOrchestrationProgress((record) => {
            record[storageKey] = choice;
            record[passedKey] = correct;
            reconcileAgentOrchestrationModuleCompletion(record, slug);
          });
          setSaveResult(persisted);
        }}
      >
...
export function OrchestrationLab({
  ...
  const acceptedReceipt = agentOrchestrationModuleRequirements(
    progress,
    slug,
  ).lab;
    ...
    const timer = window.setTimeout(() => {
      for (const signature of [pendingLabSignature, storedLabSignature]) {
        ...
        if (
          ...
          setState(normalizeAgentOrchestrationLabState(saved.state));
          setLearnerEvidence(
            typeof saved.learnerEvidence === "string"
      ...
      setState({ ...AGENT_ORCHESTRATION_INITIAL_LAB_STATE });
      setLearnerEvidence("");
    }, 0);
    return () => window.clearTimeout(timer);
  }, [labId, pendingLabSignature, slug, storedLabSignature]);
  ...
  const save = () => {
    ...
    setSaveResult(receiptSaved ? persisted : null);
    if (receiptSaved) setDraftResult(null);
  };

  return (
    <section className={styles.lab} aria-labelledby={`${slug}-${labId}-lab-title`}>
      ...
      <div className={styles.labGrid}>
        <div className={styles.labControls}>
          ...
          >
            {saveResult !== null || acceptedReceipt
              ? copy.savedLab
          ...
          <small
            ...
            aria-live="polite"
          >
            {acceptedReceipt && saveResult === null
              ? copy.acceptedLabReceipt
              : saveResult !== null
              ? `${copy.savedLab}. ${persistenceText(labels, saveResult)}`
              : !scenarioCompletable
                ? copy.scenarioRequired
                : !evidenceCompletable
                  ? copy.learnerEvidenceRequired
                  : draftResult !== null
                    ? `${copy.labDraftSaved} ${persistenceText(labels, draftResult)}`
                    : copy.labDraftSaved}
          </small>
        </div>
...
export function ModuleCompletion({
  ...
  const requirements = agentOrchestrationModuleRequirements(record, slug);
  const complete = isAgentOrchestrationModuleComplete(record, slug);
  const [saveResult, setSaveResult] = useState<boolean | null>(null);
  const missing = [
  ...
  return (
    <section className={styles.completion} aria-labelledby={`${slug}-completion-title`}>
      <div>
...
export function FinalAssessment({
  ...
  questions: readonly AgentOrchestrationCheckpointCopy[];
  passPercent: number;
  title: string;
  summary: string;
  labels: Labels;
  ...
  const [saveResult, setSaveResult] = useState<boolean | null>(null);
  const attemptPassed = score !== null && score >= passPercent;
  const best = typeof progress[AGENT_ORCHESTRATION_QUIZ_BEST_KEY] === "number"
    ? progress[AGENT_ORCHESTRATION_QUIZ_BEST_KEY] as number
    : 0;
  const coursePassed = isAgentOrchestrationQuizPassed(progress);

  return (
    <section className={styles.assessment} id="agent-orchestration-assessment" aria-labelledby="assessment-title">
      <header>
        <p className={styles.kicker}>{label(labels, "assessment", "Final assessment")}</p>
        <h2 id="assessment-title">{title}</h2>
        <p>{summary}</p>
        <div><span>{label(labels, "assessmentThreshold", "Pass")} ≥ {passPercent}%</span><span>{label(labels, "bestScore", "Best")} {best}%</span></div>
      </header>
            ...
            <fieldset>
              ...
              {question.options.map((option, optionIndex) => (
                <label key={option}>
                  <input
                    type="radio"
                    name={`agent-orchestration-final-${questionIndex}`}
      ...
      <button
        ...
        onClick={() => {
          ...
          setScore(nextScore);
          const persisted = updateAgentOrchestrationProgress((record) => {
            recordAgentOrchestrationQuizAttempt(record, nextScore, passPercent);
          });
          setSaveResult(persisted);
        }}
      >
      ...
      {score !== null ? (
        <div className={attemptPassed || coursePassed ? styles.feedbackPass : styles.feedbackRetry} role="status">
          <strong>
            {score}% · {attemptPassed
              ? label(labels, "passed", "Passed")
              : coursePassed
                ? label(labels, "passPreserved", isChinese(labels) ? "本次未达线；历史通过状态已保留" : "Attempt below threshold; prior pass preserved")
          ...
          <p>
            {attemptPassed
              ...
              : coursePassed
                ? label(labels, "bestPreserved", isChinese(labels) ? "最佳分数与课程通过状态都不会因较低的后续尝试而下降。" : "Your best score and passing course status remain monotonic after a lower attempt.")
                : label(labels, "assessmentRetry", "Review the boundary explanations, then try another clean trial.")}
          </p>
          <small className={styles.persistenceStatus}>
            {persistenceText(labels, saveResult)}
          </small>
...
export function CapstoneChecklist({
  ...
  const progress = useProgress();
  const evidence = agentOrchestrationCapstoneEvidence(progress, artifacts.length);
  const validation = validateAgentOrchestrationCapstoneEvidence(
    evidence,
    artifacts.length,
  );
  const evidenceValidity = validation.valid;
  const complete = validation.complete;
  const [saveResult, setSaveResult] = useState<boolean | null>(null);

  const updateEvidence = (index: number, value: string) => {
    const next = evidence.map((current, itemIndex) => itemIndex === index ? value : current);
    const nextValidation = validateAgentOrchestrationCapstoneEvidence(
      next,
      artifacts.length,
    );
    const persisted = updateAgentOrchestrationProgress((record) => {
      record[AGENT_ORCHESTRATION_CAPSTONE_CHECKS_KEY] = next;
      record[AGENT_ORCHESTRATION_CAPSTONE_KEY] = nextValidation.complete;
    });
    setSaveResult(persisted);
  };

  return (
    <section className={styles.capstoneChecklist} aria-labelledby="capstone-checklist-title">
      <header>
        <p className={styles.sectionLabel}>{label(labels, "releaseContract", "Release contract")}</p>
        <h3 id="capstone-checklist-title">{label(labels, "auditableArtifacts", "15 auditable artifacts")}</h3>
        <span>{evidenceValidity.filter(Boolean).length}/{artifacts.length}</span>
      </header>
      <ol>
        {artifacts.map((artifact, index) => (
          <li key={artifact}>
            <label>
              ...
              <span className={styles.capstoneEvidenceField}>
                <span>{label(labels, "evidenceReference", isChinese(labels) ? "证据 / 引用" : "Evidence / reference")}</span>
                <input
                  type="text"
                  ...
                  aria-label={`${artifact} — ${label(labels, "evidenceReference", isChinese(labels) ? "证据或引用" : "evidence or reference")}`}
                  aria-invalid={evidence[index].trim().length > 0 && !evidenceValidity[index]}
                  aria-describedby="capstone-evidence-guidance"
                  placeholder={label(labels, "evidencePlaceholder", isChinese(labels) ? "输入文件、URL、trace ID 或评审记录" : "File, URL, trace ID, or review record")}
                />
              </span>
            </label>
          </li>
      ...
      <p id="capstone-evidence-guidance">
        {label(
          labels,
          "evidenceGuidance",
          isChinese(labels)
            ? "每项必须使用不同的 HTTPS URL、带扩展名的文件路径、结构化 trace／ticket／review ID，或包含标识符的实质评审记录。浏览器只排除明显占位符，无法证明外部对象真实存在；最终仍须人工核验。"
      ...
      <p data-complete={complete || undefined}>{complete ? statement : label(labels, "capstoneOpen", "Completion remains open until every artifact is evidenced.")}</p>
      <small className={styles.persistenceStatus} role="status" aria-live="polite">
        {persistenceText(labels, saveResult)}
      </small>
    </section>
