    ...
    >
      ...
      <header className={styles.hero}>
        <div className={styles.heroCopy}>
          ...
          <ul className={styles.principles} aria-label="Course production principles">
      ...
      <section className={styles.sourceAtlas} aria-labelledby="agentic-video-source-atlas-title">
        <header className={styles.sectionHeader}>
          ...
          <h2 id="agentic-video-source-atlas-title">
            {course.contentLocale === "zh-Hans" ? `${githubCount} 个 GitHub 实现来源，${xCount} 个经身份核验的 X 信号` : `${githubCount} GitHub implementation sources, ${xCount} identity-verified X signals`}
          </h2>
          <p>{course.copy.meta.evidenceNote}</p>
        </header>
        <div className={styles.sourceColumns}>
          {(["github-repository", "x-post"] as const).map((kind) => (
            <section key={kind} aria-labelledby={`source-kind-${kind}`}>
              <h3 id={`source-kind-${kind}`}>{kind === "github-repository" ? label(course.copy.ui, "githubRepository", "GitHub repositories") : label(course.copy.ui, "xPost", "X field signals")}</h3>
                ...
                {sources.filter((source) => source.kind === kind).map((source) => (
                  <details key={source.id}>
                    ...
                    <div>
                      ...
                      <a href={source.url} target="_blank" rel="noopener noreferrer">{sourceType(source, course.copy.ui)}<span aria-hidden="true">↗</span></a>
                    </div>
                  </details>
                ))}
              </div>
