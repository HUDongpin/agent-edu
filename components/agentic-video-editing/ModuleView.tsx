...
import { getAgenticVideoEditingClaims } from "@/lib/agentic-video-editing";
import {
  ArtifactWorkbench,
  CutPlanLab,
  ModuleCheckpoint,
...
function evidenceModeLabel(mode: AgenticVideoEditingEvidenceMode, labels: Labels): string {
  ...
  if (mode === "instructional-synthesis") return label(labels, "instructionalSynthesis", "Instructional synthesis");
  if (mode === "course-policy") return label(labels, "coursePolicy", "Course policy");
...
export default function ModuleView({
  ...
  return (
    ...
    >
      ...
      <nav className={styles.breadcrumb} aria-label={label(course.copy.ui, "courseBreadcrumb", "Course breadcrumb")}>
      ...
      <div className={styles.moduleLayout}>
        <aside className={styles.moduleRail}>
          <nav aria-label={label(course.copy.ui, "allCourseModules", "All course modules")}>
            <header><span>{label(course.copy.ui, "course", "Course")} 22</span><strong>{label(course.copy.ui, "tableOfContents", "Course map")}</strong></header>
          ...
          <article>
            ...
            <nav className={styles.onPageNav} aria-label={label(course.copy.ui, "onThisPage", "On this page")}>
              ...
              <ol>
                ...
                {module.slug === "declarative-edit-plan" ? <li><a href="#cut-plan-lab">04 {label(course.copy.ui, "cutPlanLab", "Cut Plan Lab")}</a></li> : null}
              ...
              return (
                <section className={styles.proseSection} id={`module-section-${sectionIndex + 1}`} key={section.heading} aria-labelledby={`module-section-${sectionIndex + 1}-title`}>
                  ...
                  <div className={styles.proseCopy}>
                    <p className={styles.evidenceBadge} data-mode={section.evidenceMode}>{section.evidenceMode === "source-grounded"
                      ? label(course.copy.ui, "claimLedger", "Atomic claim ledger")
                      : evidenceModeLabel(section.evidenceMode, course.copy.ui)}</p>
                    <h2 id={`module-section-${sectionIndex + 1}-title`}>{section.heading}</h2>
                    {section.paragraphs.map((paragraph, paragraphIndex) => {
                      const claims = getAgenticVideoEditingClaims(
                        module.slug,
                        sectionIndex,
                        paragraphIndex,
                      );
                      return (
                        <div className={styles.claimBlock} key={paragraph}>
                          <p>{paragraph}</p>
                          {claims.map((claim) => {
                            const source = claim.sourceId
                              ? module.sources.find((candidate) => candidate.id === claim.sourceId)
                              : undefined;
                            return (
                              <aside className={styles.inlineEvidence} data-claim-id={claim.id} key={claim.id} aria-label={label(course.copy.ui, "atomicClaim", "Atomic claim")}>
                                <span>{evidenceModeLabel(claim.evidenceMode, course.copy.ui)}</span>
                                <p><code lang="en" dir="ltr">{claim.id}</code> · {course.contentLocale === "zh-Hans" ? claim.claimZhHans : claim.claim}</p>
                                <p><strong>{label(course.copy.ui, "exactLocator", "Exact locator")}: </strong>{claim.evidenceUrl ? (
                                  <a href={claim.evidenceUrl} target="_blank" rel="noopener noreferrer">{source?.title ?? claim.sourceId}</a>
                                ) : null}{claim.evidenceUrl ? " · " : ""}{claim.locator}</p>
                                <p><strong>{label(course.copy.ui, "boundary", "Boundary")}: </strong>{course.contentLocale === "zh-Hans" ? claim.boundaryZhHans : claim.boundary}</p>
                              </aside>
                            );
                          })}
                        </div>
            ...
            <section className={styles.practice} id="module-practice" aria-labelledby="module-practice-title">
              ...
              <ArtifactWorkbench module={module} practice={module.copy.practice} labels={course.copy.ui} />
            </section>
            ...
            <ModuleCompletion module={module} labels={course.copy.ui} />
            <nav className={styles.modulePager} aria-label={label(course.copy.ui, "adjacentModules", "Adjacent modules")}>
