...
/* Both diagrams sit on the same page, so the marker id has to differ between
   them — two <marker id="dc-tip"> would be a duplicate id, and every arrowhead
   in the second diagram would silently resolve to the first one's. */
function Defs({ id }: { id: string }) {
  return (
    <defs>
      <marker id={id} viewBox="0 0 8 8" refX="7.5" refY="4"
    ...
    <svg viewBox="0 0 288 80" className="dcsvg" role="img" aria-hidden="true">
      ...
      {xs.map((x, i) => (
        <g key={x}>
          ...
          {i < xs.length - 1 && (
            <path d={`M${x + W} 40 h9`} className="dc-edge" markerEnd={`url(#${tip})`} fill="none" />
          )}
        </g>
      ))}
    </svg>
...
export function ModelStep() {
  ...
  return (
    <svg viewBox="0 0 288 80" className="dcsvg" role="img" aria-hidden="true">
      ...
      <path d="M60 40 h9" className="dc-edge" markerEnd={`url(#${tip})`} fill="none" />
      ...
      <path d="M120 40 C133 40 133 11 142 11" className="dc-edge ghost" markerEnd={`url(#${tip})`} fill="none" />
      <path d="M120 40 h22" className="dc-edge" markerEnd={`url(#${tip})`} fill="none" />
      <path d="M120 40 C133 40 133 69 142 69" className="dc-edge ghost" markerEnd={`url(#${tip})`} fill="none" />
      ...
      <path d="M202 40 h9" className="dc-edge" markerEnd={`url(#${tip})`} fill="none" />
