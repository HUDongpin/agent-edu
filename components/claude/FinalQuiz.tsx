...
export default function FinalQuiz({
  ...
  const bankByUnit = useMemo(() => {
    const counts = new Map<ClaudeUnitId, number>();
  ...
  return (
    <section
      ...
      aria-labelledby="claude-final-quiz-title"
      data-testid="claude-final-quiz"
    >
      <header className={styles.finalQuizHeader}>
        <div>
          <p className={styles.kicker}>{labels.quiz}</p>
          <h2 id="claude-final-quiz-title" tabIndex={-1}>{labels.finalQuizTitle}</h2>
