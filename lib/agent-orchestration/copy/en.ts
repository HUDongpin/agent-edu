...
export const AGENT_ORCHESTRATION_EN_COPY = {
  ...
  ui: {
    ...
    returnToCourse: "Return to course overview",
    ...
    restoringProgress: "Restoring local progress…",
    storageCorrupt: "The shared learning record in this browser is unreadable. Its original bytes remain untouched; download a recovery copy before using the site-wide recovery tools in My Learning.",
    storageQuotaExceeded: "Browser storage is full, so new learning work cannot be saved safely. Download the recovery copy before leaving this page.",
    storageUnavailable: "Local storage is unavailable in this browser. Course controls are paused to avoid false progress or lost work.",
    downloadRecovery: "Download recovery copy",
    openRecoveryTools: "Open My Learning recovery tools",
    recoveryDownloaded: "Recovery copy downloaded; the original browser record is still unchanged.",
    recoveryUnavailable: "No recovery data is available to download.",
    workspaceEyebrow: "Portable workspace",
    workspaceTitle: "Export or restore Course 15 work",
    workspaceLocal: "Local file · never uploaded",
    workspaceIntro:
    ...
    chooseWorkspace: "Choose restore file",
    workspaceRejected: "File not accepted",
    restorePreview: "Restore preview",
    workspaceAdd: "Will add",
    workspaceSame: "Already identical",
    workspaceKeep: "Keep local",
    workspaceConflicts: "Review preserved local fields",
    applyWorkspace: "Restore new content ({add})",
    workspaceRestored:
      "Restore complete: {applied} added; {skipped} kept local.",
    workspaceRestoreFailed:
      "The restore write could not be verified; the existing browser record remains unchanged.",
    migrationPreserved: "Course update preserved {preserved} learner-authored items; {invalidated} stale receipts require review.",
    capstoneRecoveryPending: "These references were recovered after a course update, but their old receipt does not count as completion evidence. Review and resave them explicitly.",
    resetComplete: "Course 15 progress reset in this browser",
    resetFailed: "Reset could not be verified; the existing browser record remains unchanged.",
    ...
    resetCancelled: "Reset cancelled. No progress was changed.",
    resetExpired: "Reset confirmation expired. No progress was changed.",
    ...
    assessmentOf: "of",
    assessmentReadyToGrade: "All questions are answered. Ready to grade.",
    assessmentResultTitle: "Assessment result",
    assessmentWorkspaceTitle: "Check your architecture decisions",
    assessmentQuestionsToReview: "Questions to review",
    assessmentCorrectAnswer: "Correct answer",
    assessmentReviewModule: "Review module",
    assessmentRetryAction: "Try the assessment again",
    assessmentOptionSelected: "Selected",
    assessmentOptionSelectedCorrect: "Selected · correct",
    assessmentOptionCorrect: "Correct answer",
    assessmentOptionSelectedMissed: "Your answer · review",
    ...
    capstoneReviewFirst: "Review first unresolved artifact",
    capstoneAllResolved: "Every evidence field passes the local format and uniqueness gate.",
    evidenceRequired: "Add an evidence reference for this artifact.",
    evidenceTooLong: "Shorten this reference to 2,048 characters or fewer.",
    evidenceUnsupportedScheme: "Use HTTPS or an approved trace, review, ticket, file, or artifact identifier.",
    evidenceTooShort: "Add a specific reference with at least eight meaningful characters.",
    evidencePlaceholderReason: "Replace sample or temporary text with the real evidence reference.",
    evidenceMissingIdentifier: "Add a stable URL, file extension, or trace, ticket, or review identifier.",
    evidenceDuplicate: "Use a distinct reference; this evidence is already assigned to another artifact.",
    evidenceAccepted: "Reference passes the local format and uniqueness gate.",
  ...
  modules: {
    "workflow-agent-boundary": {
      ...
      checkpoint: {
        checkpointId: "ao15.workflow-agent-boundary.en.deterministic-default",
        ...
        options: [
          { id: "hierarchical-multi-agent-swarm", label: "A hierarchical multi-agent swarm" },
          { id: "code-directed-deterministic-workflow", label: "A code-directed workflow with explicit deterministic checks" },
          { id: "handoff-network-per-branch", label: "A handoff network with one agent per branch" },
          { id: "hosted-subagent-tree", label: "A hosted subagent tree because it is easier to diagram" },
        ],
        correctOptionId: "code-directed-deterministic-workflow",
        explanation:
    ...
    "task-graphs-contracts": {
      ...
      checkpoint: {
        checkpointId: "ao15.task-graphs-contracts.en.fanout-contract",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
          "Which graph definition is sufficient for a three-worker fan-out?",
        options: [
          { id: "parallel-arrows-only", label: "Three arrows from a coordinator labelled ‘work in parallel’" },
          { id: "roles-and-shared-chat", label: "Three role descriptions and a shared chat transcript" },
          { id: "typed-workers-state-limits-join-terminals", label: "Typed worker contracts, isolated state, concurrency and budget limits, a join policy, and terminal states" },
          { id: "framework-visualizer", label: "A framework name and a screenshot of its visualizer" },
        ],
        correctOptionId: "typed-workers-state-limits-join-terminals",
        explanation:
    ...
    "chaining-routing": {
      ...
      checkpoint: {
        checkpointId: "ao15.chaining-routing.en.schema-valid-route",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
          "A router returns a schema-valid destination with high confidence. What has been established?",
        options: [
          { id: "route-is-true-safe-authorized", label: "The route is true, safe, and authorized" },
          { id: "schema-conformance-only", label: "Only that the returned data conforms to the supported schema; policy and task correctness still require checks" },
          { id: "destination-bypasses-permissions", label: "The destination may bypass its own tool permissions" },
          { id: "unknown-route-unneeded", label: "No unknown route is needed" },
        ],
        correctOptionId: "schema-conformance-only",
        explanation:
    ...
    "parallel-fanout-fanin": {
      ...
      checkpoint: {
        checkpointId: "ao15.parallel-fanout-fanin.en.tool-call-intents",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
          "A model emits four function calls in one response. What can you safely infer?",
        options: [
          { id: "four-concurrent-agents", label: "Four complete agents are running concurrently" },
          { id: "tool-call-intents-only", label: "The provider returned four tool-call intents; execution concurrency and agent topology depend on the SDK and application" },
          { id: "authorized-independent-calls", label: "All four calls are authorized and independent" },
          { id: "join-unnecessary", label: "The join can be omitted because the model issued the calls together" },
        ],
        correctOptionId: "tool-call-intents-only",
        explanation:
    ...
    "manager-roles-ownership": {
      ...
      checkpoint: {
        checkpointId: "ao15.manager-roles-ownership.en.final-synthesis-owner",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
          "In a manager-with-agents-as-tools topology, who normally owns the final user-facing synthesis?",
        options: [
          { id: "last-specialist", label: "Whichever specialist finishes last" },
          { id: "manager-after-validation", label: "The manager, after validating specialist returns" },
          { id: "host-operating-system", label: "The Host operating system" },
          { id: "every-worker", label: "Every worker independently" },
        ],
        correctOptionId: "manager-after-validation",
        explanation:
    ...
    "delegation-handoffs": {
      ...
      checkpoint: {
        checkpointId: "ao15.delegation-handoffs.en.active-agent-continuity",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
          "After a handoff, a new user turn arrives. What must the application know?",
        options: [
          { id: "original-manager-name", label: "Only the original manager's name" },
          { id: "active-agent-and-runtime-state", label: "Which agent is currently active and the runtime-specific state needed to continue it" },
          { id: "host-count", label: "The total number of available Hosts" },
          { id: "restart-original-agent", label: "Nothing; every turn should restart the original agent" },
        ],
        correctOptionId: "active-agent-and-runtime-state",
        explanation:
    ...
    "orchestrator-workers-verification": {
      ...
      checkpoint: {
        checkpointId: "ao15.orchestrator-workers-verification.en.independent-verifier",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
          "What most strengthens an independent verifier?",
        options: [
          { id: "producer-rationale", label: "Giving it the producer's approval rationale and asking it to agree" },
          { id: "primary-artifacts-separate-checks-fail-unknown", label: "Allowing it to inspect primary artifacts, run separate checks, and return fail or unknown" },
          { id: "producer-summary-only", label: "Using the same summary as the only evidence" },
          { id: "unbounded-review", label: "Removing the stopping rule so it can keep reviewing" },
        ],
        correctOptionId: "primary-artifacts-separate-checks-fail-unknown",
        explanation:
    ...
    "tools-aci-mcp": {
      ...
      checkpoint: {
        checkpointId: "ao15.tools-aci-mcp.en.protocol-scope",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question: "What does MCP provide by itself?",
        options: [
          { id: "complete-orchestrator", label: "A complete agent orchestrator with memory, sandboxing, and evals" },
          { id: "declared-capability-exchange", label: "A standardized way to exchange declared capabilities and related protocol data" },
          { id: "automatic-authorization", label: "Automatic authorization for every connected tool" },
          { id: "safe-remote-output", label: "A guarantee that remote server output is safe" },
        ],
        correctOptionId: "declared-capability-exchange",
        explanation:
    ...
    "context-state-memory": {
      ...
      checkpoint: {
        checkpointId: "ao15.context-state-memory.en.compaction-boundary",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question: "Which statement about compaction is safest?",
        options: [
          { id: "complete-audit-record", label: "A compacted context is a complete audit record" },
          { id: "continuation-with-separate-state-evidence", label: "Compaction can support continuation, but durable task state and evidence must remain separately inspectable" },
          { id: "versioned-state-unnecessary", label: "Compaction makes versioned state unnecessary" },
          { id: "credentials-safe", label: "A compacted summary can safely contain every credential" },
        ],
        correctOptionId: "continuation-with-separate-state-evidence",
        explanation:
    ...
    "budgets-concurrency-stopping": {
      ...
      checkpoint: {
        checkpointId: "ao15.budgets-concurrency-stopping.en.runtime-capacity",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question: "A runtime exposes eight open slots. What does that establish?",
        options: [
          { id: "optimal-eight-agent-team", label: "The optimal team has eight agents" },
          { id: "eight-agent-host", label: "The Host is composed of eight agents" },
          { id: "capacity-not-policy", label: "One runtime currently exposes an active-work capacity; policy, budget, independence, and evaluation still determine useful concurrency" },
          { id: "spawn-eight-descendants", label: "Every task should immediately spawn eight descendants" },
        ],
        correctOptionId: "capacity-not-policy",
        explanation:
    ...
    "reliability-recovery": {
      ...
      checkpoint: {
        checkpointId: "ao15.reliability-recovery.en.ambiguous-payment-timeout",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
          "A payment call times out after the server may have committed it. What is the safest next action?",
        options: [
          { id: "repeat-new-operation-id", label: "Repeat the payment with a new operation ID immediately" },
          { id: "assume-failure", label: "Assume failure because no response arrived" },
          { id: "reconcile-original-operation-id", label: "Query or reconcile using the original stable operation ID before attempting any new effect" },
          { id: "model-guess", label: "Ask the model whether the payment probably succeeded" },
        ],
        correctOptionId: "reconcile-original-operation-id",
        explanation:
    ...
    "security-authority-human-control": {
      ...
      checkpoint: {
        checkpointId: "ao15.security-authority-human-control.en.approval-scope",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
          "A human approves a state-changing tool call. What does that approval establish?",
        options: [
          { id: "approval-proves-correctness", label: "The action is correct and the output passed quality review" },
          { id: "scoped-action-only", label: "The approved action may proceed within its recorded scope; correctness, containment, execution evidence, and sign-off remain separate" },
          { id: "permanent-descendant-permission", label: "Every descendant agent inherits permission forever" },
          { id: "guardrail-undoes-effects", label: "An output guardrail can undo any side effect" },
        ],
        correctOptionId: "scoped-action-only",
        explanation:
    ...
    "tracing-observability-economics": {
      ...
      checkpoint: {
        checkpointId: "ao15.tracing-observability-economics.en.trace-boundary",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
          "A run has a complete trace with no errors. What can the team conclude?",
        options: [
          { id: "trace-proves-outcome", label: "The answer was correct and the user outcome succeeded" },
          { id: "recorded-path-no-traced-error", label: "The recorded execution path completed without a traced error; quality and outcome still need evaluation evidence" },
          { id: "audit-unnecessary", label: "No audit record is required" },
          { id: "cost-effective", label: "The run was cost-effective" },
        ],
        correctOptionId: "recorded-path-no-traced-error",
        explanation:
    ...
    "evaluation-regression-evolution": {
      ...
      checkpoint: {
        checkpointId: "ao15.evaluation-regression-evolution.en.unsafe-trajectory",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
        ...
        options: [
          { id: "pass-terminal-answer", label: "Pass because only the final answer matters" },
          { id: "block-unsafe-trajectory", label: "Fail or block according to the trajectory and safety criteria, even though the terminal answer is correct" },
          { id: "delete-trace-rerun", label: "Delete the trace and rerun once" },
          { id: "cost-only", label: "Count it as a cost-only issue" },
        ],
        correctOptionId: "block-unsafe-trajectory",
        explanation:
    ...
    "production-orchestration-capstone": {
      ...
      checkpoint: {
        checkpointId: "ao15.production-orchestration-capstone.en.staging-response-proof",
        contentVersion: AGENT_ORCHESTRATION_CHECKPOINT_CONTENT_VERSION,
        question:
          "A background agent response completes successfully in a staging demo. What has been proven?",
        options: [
          { id: "production-workflow-proven", label: "The complete multi-step business workflow is durable and production-ready" },
          { id: "one-staged-response-only", label: "That one staged response completed; production still requires application state, authority, recovery, eval, monitoring, and release evidence" },
          { id: "exactly-once-webhooks", label: "Webhooks will be delivered exactly once" },
          { id: "approval-rollback-unneeded", label: "Human approval and rollback are no longer needed" },
        ],
        correctOptionId: "one-staged-response-only",
        explanation:
