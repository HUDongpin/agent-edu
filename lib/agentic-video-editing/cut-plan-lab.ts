...
/**
 * SHA-256 of `JSON.stringify(CUT_PLAN_LAB_FIXTURE)`. The M5 teaching plan
 * consumes this bounded candidate-set snapshot; the three analysis lanes that
 * produced comparable candidates belong to M4's separate artifact contract.
 */
export const CUT_PLAN_LAB_CANDIDATE_SET_SHA256 =
  "ee16eac9a48eaf0c80a7e4a2774e216481a1cee29481353b83b9a64a756dc5ef" as const;
export const CUT_PLAN_LAB_CANDIDATE_SET_LOCATOR =
  "fixture:course22-cut-plan-lab-v2:candidate-segments" as const;
...
export interface CutPlanLabPlan {
  ...
  readonly fixtureId: "course22-cut-plan-lab-v2";
  readonly planMode: "teaching-fixture";
  readonly executionMode: "select-only";
  ...
  readonly timeline: {
    ...
    readonly operations: readonly {
      ...
      readonly evidence: readonly [{
        ...
        readonly artifactId: "course22-cut-plan-lab";
  ...
  readonly executionPolicy: {
    ...
    readonly outputDirectory: "edits/course22-lab/";
    readonly stopOnInputHashMismatch: true;
    readonly stopOnUnresolvedAmbiguity: true;
  };
  readonly candidateSetRef: {
    readonly projectId: "course22-cut-plan-lab-v2";
    readonly artifactId: "candidate-segments";
    readonly artifactSchemaId: "aicourse.agentic-video-editing.module.semantic-analysis-director.artifact.v2";
    readonly artifactSha256: typeof CUT_PLAN_LAB_CANDIDATE_SET_SHA256;
    readonly locator: typeof CUT_PLAN_LAB_CANDIDATE_SET_LOCATOR;
    readonly selectedLaneId: "recorded-fixture";
  };
  readonly completion: {
    readonly requiredFieldPaths: readonly string[];
    readonly closedFieldPaths: readonly string[];
    readonly complete: boolean;
  };
}

export const CUT_PLAN_LAB_REQUIRED_FIELD_PATHS = [
  "schemaVersion",
  "planId",
  "fixtureId",
  "planMode",
  ...
  "status",
  "inputs[0].mediaId",
  "inputs[0].manifestDecision",
  "timeline.timebase",
  "timeline.operations",
  "executionPolicy.dryRunRequired",
  "executionPolicy.outputDirectory",
  "candidateSetRef",
] as const;
...
export function buildCutPlanLabPlan(
  ...
  const operations = clips.map((clip) => {
    ...
    const operation = {
      ...
      evidence: [{
        ...
        artifactId: "course22-cut-plan-lab" as const,
        artifactSha256: null,
  ...
  return {
    ...
    planId: "course22-lab-plan-v2",
    fixtureId: "course22-cut-plan-lab-v2",
    planMode: "teaching-fixture",
    executionMode: "select-only",
    status: "blocked",
    inputs: [{
    ...
    executionPolicy: {
      ...
      allowNetwork: false,
      allowPublish: false,
      outputDirectory: "edits/course22-lab/",
      stopOnInputHashMismatch: true,
      stopOnUnresolvedAmbiguity: true,
    },
    candidateSetRef: {
      projectId: "course22-cut-plan-lab-v2",
      ...
      artifactSha256: CUT_PLAN_LAB_CANDIDATE_SET_SHA256,
      locator: CUT_PLAN_LAB_CANDIDATE_SET_LOCATOR,
      selectedLaneId: "recorded-fixture",
    },
    completion: {
      requiredFieldPaths: CUT_PLAN_LAB_REQUIRED_FIELD_PATHS,
      closedFieldPaths: CUT_PLAN_LAB_REQUIRED_FIELD_PATHS,
      complete: true,
    },
  };
}
...
export function validateCutPlanLabPlan(plan: CutPlanLabPlan): CutPlanLabIssue[] {
  ...
  if (plan.planMode !== "teaching-fixture" || plan.status !== "blocked"
    || plan.fixtureId !== "course22-cut-plan-lab-v2"
    || plan.executionMode !== "select-only"
  ...
  if (plan.candidateSetRef.projectId !== "course22-cut-plan-lab-v2"
    || plan.candidateSetRef.artifactId !== "candidate-segments"
    || plan.candidateSetRef.artifactSchemaId
      !== "aicourse.agentic-video-editing.module.semantic-analysis-director.artifact.v2"
    || plan.candidateSetRef.artifactSha256 !== CUT_PLAN_LAB_CANDIDATE_SET_SHA256
    || plan.candidateSetRef.locator !== CUT_PLAN_LAB_CANDIDATE_SET_LOCATOR
    || plan.candidateSetRef.selectedLaneId !== "recorded-fixture") {
    issues.push({ code: "candidate-set-ref" });
  }
  const requiredFields = [...CUT_PLAN_LAB_REQUIRED_FIELD_PATHS].sort();
  const declaredRequiredFields = [...plan.completion.requiredFieldPaths].sort();
  const closedFields = [...plan.completion.closedFieldPaths].sort();
  const fieldsClosed = JSON.stringify(requiredFields) === JSON.stringify(declaredRequiredFields)
    && JSON.stringify(requiredFields) === JSON.stringify(closedFields);
  if (!fieldsClosed || plan.completion.complete !== fieldsClosed) {
    issues.push({ code: "completion-fields-open" });
  }
  ...
  for (const operation of operations) {
    ...
    if (evidence.length !== 1
      ...
      || evidence[0]?.artifactId !== "course22-cut-plan-lab"
