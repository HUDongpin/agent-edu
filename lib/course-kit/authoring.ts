...
export type CourseKitSourceAuthoringRecord<SourceId extends string = string> =
  Omit<CourseKitSourceRecord<SourceId>, "schemaVersion"> & {
...
export interface CourseKitSourceAuthoringSeed<SourceId extends string = string> {
  /** `supports` and `boundary` are the reviewed English annotations. */
  ...
  return {
    ...
    meta: {
      ...
      fallbackNotice:
        ...
        (english
          ? "This course has not yet been reviewed in your selected language. The reviewed English edition is shown left to right."
          : "本课程提供经过审核的简体中文版本。"),
...
> {
  ...
  const capstone: CourseKitCapstone<ArtifactId, SourceId> = {
