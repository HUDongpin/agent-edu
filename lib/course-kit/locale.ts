...
import { materialiseCourseKitSourcePresentation } from "./source-presentation";
import {
...
export function resolveCourseKitLocale(
  ...
  const hasNativeContent = COURSE_KIT_CONTENT_LOCALES.some(
    (locale) => locale === requestedLocale,
  );
  const contentLocale: CourseKitContentLocale = hasNativeContent
    ? (requestedLocale as CourseKitContentLocale)
    : COURSE_KIT_FALLBACK_LOCALE;
  ...
  return {
    ...
    contentLocale,
    // Both complete course copies use an LTR content surface. The surrounding
...
export function materialiseCourseKit(
  ...
  const sources = definition.sources.map((source) =>
    materialiseCourseKitSourcePresentation(
      source,
      copy.sourceAnnotations[source.id],
  ...
  return {
    ...
    capstone: {
      ...
      responsibleAiRubric: [...copy.capstone.responsibleAiRubric],
      responsibleAiGate: definition.capstone.responsibleAiGate
        ? {
            version: definition.capstone.responsibleAiGate.version,
            criteria: definition.capstone.responsibleAiGate.criteria.map(
              (criterion) => ({
                ...criterion,
                questionIds: [...criterion.questionIds],
                artifactIds: [...criterion.artifactIds],
              }),
            ) as unknown as NonNullable<
              CourseKitMaterialisedCourse["capstone"]["responsibleAiGate"]
            >["criteria"],
          }
        : undefined,
