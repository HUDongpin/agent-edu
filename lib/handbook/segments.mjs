 ...
 *
 *   attr — a reader-facing attribute value: aria-label, title or placeholder.
 *          Keyed `hb.attr.<container>.<ordinal>`, counted separately from body
 *          text so neither renumbers the other. These were never extractable at
 *          all, which is how seventeen diagram descriptions came to be English
 *          in all nine locales while the gate reported 542 of 542: the count was
 *          right about the strings that had keys, and there were none for these.
 *
...
/**
 * The same, for a value that will sit inside double quotes.
 *
 * `escapeText` leaves a quote alone, which is correct between tags and fatal
 * inside one: a translated aria-label containing " would close the attribute
 * and put the rest of the sentence into the tag as garbage attributes.
 */
...
/**
 * Attributes a translator owns.
 *
 * Closed on purpose. These three are read aloud or shown to a reader; every
 * other attribute in this markup is structure, an id, or a class, and putting
 * one in a translator's file would be inviting them to break the page.
 */
const TRANSLATABLE_ATTRS = new Set(["aria-label", "title", "placeholder"]);
 ...
 * @returns {{name: string, id: string|null, i18n: string|null, end: number, selfClosing: boolean,
 *            attrs: {name: string, text: string, start: number, end: number}[]}}
 */
function readTag(html, lt) {
  ...
  /** @type {{name: string, text: string, start: number, end: number}[]} */
  const attrs = [];
  ...
  for (;;) {
    ...
    if (html[i] === "=") {
      ...
      if (TRANSLATABLE_ATTRS.has(attr) && translatable(value)) {
        attrs.push({ name: attr, text: decodeText(value, vs), start: vs, end: vs + value.length });
      }
    }
    if (attr === "id") id = value.trim() || null;
    else if (attr === "data-i18n") i18n = value.trim() || null;
  }
  return { name, id, i18n, end: i, selfClosing, attrs };
}
...
export function walkHandbook(html) {
  ...
  while (i < html.length) {
    ...
    /* Emitted before the tag is pushed, so the container is the enclosing
       section rather than the element itself — stable either way, because the
       markup is frozen, and it keeps a self-identifying element from owning a
       key nothing else in its scope shares. */
    for (const a of tag.attrs) {
      const id = container();
      if (!SAFE_ID.test(id)) {
        throw new Error(`segments: id "${id}" cannot be part of a dotted key`);
      }
      const ordinal = (attrCounts.get(id) ?? 0) + 1;
      attrCounts.set(id, ordinal);
      const key = `hb.attr.${id}.${String(ordinal).padStart(2, "0")}`;
      if (seen.has(key)) throw new Error(`segments: duplicate key ${key} — is an id repeated?`);
      seen.add(key);
      segments.push({ kind: "attr", key, text: a.text, start: a.start, end: a.end });
    }
