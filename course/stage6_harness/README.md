...
`run.py` runs the same job four times, switching one part off each time. The model, the prompt and the tools are **identical** in every run. Only the code around them changes.
...
**Next:** [stage 7 — guarantee the check happens](../stage7_graph/README.md)
