...
本节保留较宽的前期候选调研，用于比较技术路径、维护风险与许可证模式；分组表示当时的研究优先级，不是 Course 16 的发布状态。唯一内部采用裁决是第十二节的 27 条 canonical ledger 与 `staging/course-src/creator-ops/lib/sources.ts`：**13 PASS / 10 CONDITIONAL / 4 EXCLUDED**。因此，本节出现但未进入 canonical ledger 的仓库只是背景或替代方案；本节提到的 FFmpeg 与 Langfuse 也必须服从 canonical ledger 的 `CONDITIONAL`，不能被解释为 `PASS`。
...
课程候选包已在 `staging/course-assets/creator-ops/lab/` 完成内部交付；本轮公开接入冻结期间，它不位于 `public/`、不生成公开链接，也不进入静态导出。该包完全由原创合成材料组成，采用 `CC0-1.0`，不需要账号、凭证、网络、付费 API 或真实平台写入：
...
本附录是 Course 16 `creator-ops` staged 检查器使用的机器可审计索引，也是课程采用状态的唯一文档裁决。它覆盖前文的候选调研分组；模块 slug 与来源标识严格对应课程清单和 `staging/course-src/creator-ops/lib/sources.ts` 在 2026-08-26 的 authored 快照。来源状态只表示内部课程采用边界，不代表公开发布：`PASS` 可在限定实验中采用，`CONDITIONAL` 必须满足附加条件，`EXCLUDED` 仅作为许可证／授权反例，不得安装、调用、打包、改编或执行。
