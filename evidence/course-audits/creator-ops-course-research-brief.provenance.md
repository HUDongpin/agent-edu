...
### P41｜Course 16 staged 候选已提供可离线复跑的原创合成验收材料

- **关键结论**：`staging/course-assets/creator-ops/lab/` 已交付四份冲突来源、33 条合成事件、10 条合成反馈、六类故障和六个 mock 发布情境；全部为原创合成材料、`CC0-1.0`、无需网络或凭证，并由十文件 SHA-256 清单锁定。冻结期间没有公开页面或静态 URL 链接这些材料。
- **课程判定**：可用于模块 2–3、5、8–10 与 Capstone 的确定性离线核心实验。
- **直接来源**：
  - `staging/course-assets/creator-ops/lab/README.md`
  - `staging/course-assets/creator-ops/lab/manifest.sha256`
  - `staging/course-assets/creator-ops/lab/fault-injections.json`
  - `staging/course-assets/creator-ops/lab/mock-publish-scenarios.json`
  - `staging/course-assets/creator-ops/lab/synthetic-events.csv`
  - `staging/course-assets/creator-ops/lab/synthetic-feedback.jsonl`
- **核验日期**：2026-08-26
- **核验说明**：staged 离线检查器验证十文件逐项 SHA-256、课程结构，以及公开路由和 `public/` 资产均为零；浏览器公开链接检查要等冻结解除并通过 release manifest 审核后才会启用。
...
最终课程台账以 `staging/course-src/creator-ops/lib/sources.ts` 的 27 条固定 revision 为唯一内部采用判定源，完整统计为 **13 PASS / 10 CONDITIONAL / 4 EXCLUDED**，并采用研究侧车中更保守的边界：
