...
The machine-readable evidence ledger is implemented in `lib/cursor/sources.ts`; the figure ledger is implemented in `lib/cursor/figures.ts`. The detailed human-readable provenance record is in `evidence/course-audits/cursor-course-research-brief.provenance.md`.
