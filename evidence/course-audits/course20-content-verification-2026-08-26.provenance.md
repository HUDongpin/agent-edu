...
> **历史快照说明（2026-08-28 更正）**：本文对应 v1.1.0 的 pre-commit 本地运行。该候选后来提交为 `35ed70003b78b3f398058ee741a1d5c6f5694183`，并推送到 `origin/codex/course-20-agentic-video-editing`。原执行目录 `/private/tmp/aicourse-agentic-video-editing` 已不存在；不得把下面的旧命令结果当作 v1.2.0、当前工作树或生产部署证据。本次复核临时副本为 `/private/tmp/course20-first-principles-final`，分支为 `codex/course-20-first-principles-fix`，且保持未 commit、未 push；永久身份应由 commit/ref 与 dirty diff 表达。
...
以下命令当时在现已移除的 `/private/tmp/aicourse-agentic-video-editing` 运行；如需复核旧快照，应从 `35ed70003b78b3f398058ee741a1d5c6f5694183` 创建新的隔离工作树，而不是假定该路径仍然存在：
...
本 provenance 只支持“v1.1.0 / `35ed700` 的前身工作副本曾在所列本地代码、内容、来源、schema、静态产物与浏览器验收范围内通过”。它不支持 v1.2.0 当前状态，也不支持“生产已部署”“真实媒体已清权”“真实剪辑质量已验证”“第三方服务当前稳定”“智能体可无人值守发布”或“课程完成等同正式证书/授权”。
